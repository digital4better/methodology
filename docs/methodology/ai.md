...
| **Génération de vidéo**<br/>(image par image)   | $FLOP \approx 2 \times P_\text{active} \times N_\text{activation} \times F$                                    | $N_{activation}$: nombre d'activations = largeur x hauteur x nombre de canaux<br/>$F$: nombre d'images à générer                                                                                                                                   | La génération traite chaque frame indépendamment<br/>(Source : Clockwork Diffusion[^2], Transformers Inference Arithmetic)                                                                                                                                                                       |
| **Génération de vidéo**<br/>(spatio-temporelle) | $FLOP \approx 2 \times P_\text{active} \times N_\text{activation} \times F + 2 \times (F \times S)^2 \times d$ | $N_{activation}$: nombre d'activations = largeur x hauteur x nombre de canaux<br/>$F$: nombre d'images à générer<br/>$S$ : nombre de tokens spatiaux = largeur x hauteur<br/>$D$ : dimension latente = nombre de canaux<br/>$d$ : dimension cachée | Le premier terme correspond au coût linéaire de génération des frames. Le second modélise le coût quadratique dominant de l’auto-attention spatio-temporelle sur l’ensemble des tokens vidéo.<br/>(Source : Video Killed the Energy Budget[^17])                                                 |
