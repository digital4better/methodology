...
| **Génération d'image**                          | $FLOP \approx 2 \times P_\text{active} \times N_\text{activation}$                                                                    | $N_{activation}$: nombre d'activations = largeur x hauteur x nombre de canaux                                                                                                                                                                                                                | Pour chaque activation et paramètre il faut 2 FLOPs pour la passe forward<br/>(Source : Clockwork Diffusion[^2], Transformers Inference Arithmetic)                                                                                                                                              |
| **Génération d'audio**<br/>(temporelle)         | $FLOP \approx S \times \big( 2 \times P_\text{active} \times N_\text{audio} + 2 \times T^2 \times d \big)$                            | $N_{audio}$ : nombre d'activations audio latentes par étape<br/>$T$ : nombre de tokens audio temporels<br/>$d$ : dimension cachée<br/>$S$ : nombre d'étapes de débruitage                                                                                                                    | L’audio est généré par diffusion sur une séquence 1D. Le coût est linéaire pour le traitement latent et quadratique pour l’auto-attention temporelle, à chaque étape de débruitage.<br/>(Sources : AudioLM[^18], MusicLM[^19], Stable Audio Open[^20])                                           |
| **Génération de vidéo**<br/>(image par image)   | $FLOP \approx S \times \big( 2 \times P_\text{active} \times N_\text{activation} \times F \big)$                                      | $N_{activation}$: nombre d'activations = largeur x hauteur x nombre de canaux<br/>$F$: nombre d'images à générer<br/>$S$ : nombre d'étapes de débruitage                                                                                                                                     | La génération traite chaque frame indépendamment<br/>(Source : Clockwork Diffusion[^2], Transformers Inference Arithmetic)                                                                                                                                                                       |
| **Génération de vidéo**<br/>(spatio-temporelle) | $FLOP \approx S \times \big( 2 \times P_\text{active} \times N_\text{activation} \times F + 2 \times (F \times S_p)^2 \times d \big)$ | $N_{activation}$: nombre d'activations = largeur x hauteur x nombre de canaux<br/>$F$: nombre d'images à générer<br/>$S$ : nombre d'étapes de débruitage<br/>$S_p$ : nombre de tokens spatiaux = largeur x hauteur<br/>$D$ : dimension latente = nombre de canaux<br/>$d$ : dimension cachée | Le premier terme correspond au coût linéaire de génération des frames. Le second modélise le coût quadratique dominant de l’auto-attention spatio-temporelle sur l’ensemble des tokens vidéo.<br/>(Source : Video Killed the Energy Budget[^17])                                                 |
