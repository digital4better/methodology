...
| **Video generation**<br/>(spatio-temporal) | $FLOP \approx S \times \big( 2 \times P_\text{active} \times N_\text{activation} \times F + 2 \times (F \times T)^2 \times d \big)$ | $N_{activation}$: number of activations = width x height x number of channels<br/>$F$: number of frames to generate<br/>$S$: number of denoising steps<br/>$T$: number of spatial tokens = width x height<br/>$D$: latent dimension = number of channels<br/>$d$: hidden dimension | The first term corresponds to the linear cost of frame generation. The second models the dominant quadratic cost of spatio-temporal self-attention across all video tokens. (Source: Video Killed the Energy Budget)                                         |
