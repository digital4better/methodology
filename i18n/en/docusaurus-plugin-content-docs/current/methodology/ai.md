...
A *token* is the discrete unit manipulated by the model to represent inputs or outputs. What a token means depends on the modality: it can be a word fragment, a spatial position, or a temporal unit produced by a codec. The key for the formulas is to use a consistent token count for the data actually processed. The table below provides quick guidance for each modality and a simple way to estimate the variables used in the formulas.

| Modality | What a token represents | Example / estimation |
|---|---|---|
| Text | Word fragment (often ~3–4 characters on average) | 100 words ≈ 130–160 tokens depending on the tokenizer. In the formulas, $T_{input}$ and $T_{output}$ come directly from the model tokenizer. |
| Image | Spatial/latent token (reduced latent space or patches) | 512×512 image, downsampling 8, 4 latent channels → latent size 64×64, so $N_{activation}=64\\times64\\times4$. With 16×16 patches: 512/16 = 32 tokens per axis → 32×32 = 1024 tokens (multiply by channels if needed). |
| Audio | Temporal token produced by a codec (e.g., EnCodec) | Estimate: $T = \\text{duration (s)} \\times \\text{sample rate} \\div \\text{downscale} \\times \\text{latent channels}$. Example: 10 s, 24 kHz, downscale 320, 8 latent channels → $T = 6000$ tokens. |
| Video | Spatial tokens per frame + number of frames | 4 s at 24 fps → $F=96$; if each frame is 512×512 with 16×16 patches, then $T$ (spatial) = 32×32 = 1024 tokens per frame. |
...
| **Video generation**<br/>(frame by frame)      | $FLOP \approx S \times \big( 2 \times P_\text{active} \times N_\text{activation} \times F \big)$               | $N_{activation}$: number of activations = width x height x number of channels<br/>$F$: number of frames to generate<br/>$S$: number of denoising steps                                                                                              | Generation processes each frame independently (Source: Clockwork Diffusion, Transformers Inference Arithmetic)                                                                                                                                                                                  |
| **Video generation**<br/>(spatio-temporal)     | $FLOP \approx S \times \big( 2 \times P_\text{active} \times N_\text{activation} \times F + 2 \times (F \times S_p)^2 \times d \big)$ | $N_{activation}$: number of activations = width x height x number of channels<br/>$F$: number of frames to generate<br/>$S$: number of denoising steps<br/>$S_p$: number of spatial tokens = width x height<br/>$D$: latent dimension = number of channels<br/>$d$: hidden dimension | The first term corresponds to the linear cost of frame generation. The second models the dominant quadratic cost of spatio-temporal self-attention across all video tokens. (Source: Video Killed the Energy Budget)                                                                            |
| **Audio generation**<br/>(temporal)            | $FLOP \approx S \times \big( 2 \times P_\text{active} \times N_\text{audio} + 2 \times T^2 \times d \big)$     | $N_{audio}$: number of latent audio activations per step<br/>$T$: number of temporal audio tokens<br/>$d$: hidden dimension<br/>$S$: number of denoising steps                                                                                     | Audio is generated by diffusion over a 1D sequence. Cost is linear for latent processing and quadratic for temporal self-attention, at each denoising step. (Sources: AudioLM, MusicLM, Stable Audio Open)                                                                                       |
