...
| **Audio** | Temporal token produced by a codec (e.g., EnCodec) | $T_\text{audio} = \text{duration (s)} \times \text{sample rate} \div \text{temporal downscale}$         | 10 s clip, 24 kHz sample rate, temporal downscale 320 → $T_\text{audio} \approx 750$ temporal tokens.                                                                                           |
| **Video** | Spatial token per frame + number of frames         | $T_\text{frame} = (\text{width}/\text{patch}) \times (\text{height}/\text{patch})$                                           | 4 s at 24 fps → $F=96$ frames. 512×512 frame, 16×16 patches → $T_\text{frame} \approx 32\times32=1{,}024$ tokens per frame and $T_\text{video} = 96 \times 1{,}024 = 98{,}304$ tokens.     |
...
| **Prompt processing**<br/>(image)          | $FLOP \approx 1 \times P_{active} \times N_\text{image}$                                                                       | $P_{active}$: number of active parameters<br/>$N_\text{image}$: number of image tokens or latent sites                                                                                                                                                       | Each prompt image is encoded once by the model. $N_\text{image}$ corresponds to encoded spatial positions, without multiplying again by the model's internal channels. (Sources: Latent Diffusion Models, DiT)                                                                                                                          |
| **Prompt processing**<br/>(audio)          | $FLOP \approx 1 \times P_{active} \times N_\text{audio}$                                                                            | $P_{active}$: number of active parameters<br/>$N_\text{audio}$: number of latent temporal audio tokens                                                                                                                                      | Each prompt audio clip is encoded once by the model. $N_\text{audio}$ corresponds to latent temporal positions, without multiplying again by the model's internal channels. (Sources: AudioLDM, Stable Audio Open)                                                                                                                   |
...
| **Image generation**<br/>(U-Net)           | $FLOP \approx S \times \big(2 \times P_\text{active} \times N_\text{site}\big)$                                                                  | $N_{site}$: number of latent spatial sites = latent width x latent height<br/>$S$: number of denoising steps                                                                                                                                                                                                      | The dominant cost of U-Net latent image diffusion is repeated denoising over a latent spatial grid. Channels are already represented in model weights and are therefore not multiplied a second time. (Sources: Latent Diffusion Models, Clockwork Diffusion, Transformers Inference Arithmetic)                                                                                                                  |
| **Image generation**<br/>(DiT)             | $FLOP \approx S \times \big(2 \times P_\text{active} \times N_\text{token} + 4 \times L \times N_\text{token}^2 \times d\big)$                  | $N_{token}=\lceil W_{latent}/p\rceil\times\lceil H_{latent}/p\rceil$<br/>$p$: latent spatial patch size<br/>$L$: number of Transformer layers<br/>$d$: hidden dimension<br/>$S$: number of denoising steps                                                                                                                       | Image DiT models process patchified latent tokens. The first term covers the model's linear pass, while the second covers quadratic attention across spatial tokens. (Sources: DiT, Latent Diffusion Models, Transformers Inference Arithmetic) |
| **Video generation**<br/>(U-Net / local frame processing) | $FLOP \approx S \times \big( 2 \times P_\text{active} \times N_\text{site} \times F \big)$ | $N_{site}$: number of latent spatial sites = latent width x latent height<br/>$F$: number of frames to generate<br/>$S$: number of denoising steps | The order of magnitude is linear with the number of frames. It represents video U-Net architectures whose dominant cost remains local to frames or small temporal neighborhoods. (Sources: Clockwork Diffusion, Transformers Inference Arithmetic, Video Killed the Energy Budget) |
| **Video generation**<br/>(DiT / global spatio-temporal attention) | $FLOP \approx S \times \big( 2 \times P_\text{active} \times N_\text{token} + 4 \times L \times N_\text{token}^2 \times d \big)$ | $N_{token}=F_{latent}\times\lceil W_{latent}/p\rceil\times\lceil H_{latent}/p\rceil$<br/>$F_{latent}=\lfloor(F-1)/c_t\rfloor+1$<br/>$p$: latent spatial patch size<br/>$c_t$: temporal compression<br/>$L$: number of Transformer layers<br/>$d$: hidden dimension | The first term corresponds to the linear cost over patchified video tokens. The second models the quadratic cost of global spatio-temporal self-attention across these tokens. (Sources: DiT, Video Killed the Energy Budget, Wan 2.1 configuration) |
| **Video generation**<br/>(Hybrid DiT + 3D VAE) | $F_\text{effective} = \lceil F / B \rceil \times B$<br/>$FLOP \approx S \times \big( 2 \times P_\text{active} \times N_\text{token} + 4 \times L \times N_\text{token}^2 \times d \big)$ | $B$: frame block size effectively computed<br/>$N_{token}$ is calculated as in the DiT case, using $F_{effective}$ instead of $F$ | Some hybrid architectures process video in fixed temporal blocks and use a 3D VAE. The cost is therefore estimated from the frames and tokens effectively computed, not only from the requested duration. (Sources: CogVideoX, Wan 2.1 configuration, Video Killed the Energy Budget) |
| **Audio generation**<br/>(diffusion / temporal attention) | $N_\text{audio}=\text{duration}\times\text{sample rate}\div c_t$<br/>$FLOP \approx S \times \big( 2 \times P_\text{active} \times N_\text{audio} + 4 \times L \times N_\text{audio}^2 \times d \big)$ | $c_t$: preset temporal downscale<br/>$N_{audio}$: number of latent temporal audio tokens<br/>$L$: number of Transformer layers<br/>$d$: hidden dimension<br/>$S$: number of denoising steps | Audio is generated by diffusion over a 1D sequence of latent tokens. Cost is linear for sequence processing and quadratic when the architecture applies global temporal attention. (Sources: AudioLDM, Stable Audio Open, Attention Is All You Need) |
| **Audio generation**<br/>(codec / autoregressive) | $N_\text{codec}=\text{duration}\times\text{sample rate}\div c_t\times C$<br/>$FLOP \approx 2 \times P_\text{active} \times N_\text{codec}$ | $c_t$: preset temporal downscale<br/>$C$: preset number of codebooks<br/>$N_{codec}$: number of generated codec tokens | Some audio generators directly produce a sequence of codec tokens. The main cost is then linear with the number of generated tokens. (Sources: MusicGen, Stable Audio Open) |
...
[^18]: Zalan Borsos, Raphael Marinier, Damien Vincent, ..., 2023. AudioLM
[^19]: Andrea Agostinelli, Timo I. Denk, Zalán Borsos, ..., 2023. MusicLM
[^20]: Zach Evans, Julian D. Parker, ..., 2024. Stable Audio Open
[^21]: William Peebles, Saining Xie, 2023. Scalable Diffusion Models with Transformers
[^22]: Alibaba Wan Team, 2025. Wan 2.1 T2V 1.3B configuration
[^23]: Hugging Face Diffusers. CogVideoXTransformer3DModel
[^24]: Robin Rombach, Andreas Blattmann, ..., 2022. High-Resolution Image Synthesis with Latent Diffusion Models
[^25]: Haohe Liu, Zehua Chen, ..., 2023. AudioLDM: Text-to-Audio Generation with Latent Diffusion Models
[^26]: Ashish Vaswani, Noam Shazeer, ..., 2017. Attention Is All You Need
[^27]: Jade Copet, Felix Kreuk, ..., 2023. Simple and Controllable Music Generation
